...
use crate::formats::{err_from_str, file_stem, parse_error_to_io_error, parse_tesla_timestamp};
use std::ffi::OsStr;
use std::io::Write;
...
impl SentryClip {
    ...
    pub fn files_per_camera(&self, camera: &Camera) -> Vec<&CameraFile> {
        self.clips
            .iter()
            .filter(|f| f.camera.eq(camera))
            .collect::<Vec<&CameraFile>>()
    }

    pub fn concatenate_camera_files(&self, camera: &Camera) -> io::Result<String> {
        let files = &self.files_per_camera(&camera);
        let result_file = self.folder.join(format!(
            "{}-{}.mp4",
            self.when.format("%Y-%m-%d_%H-%M-%S"),
            &camera.camera_file_name()
        ));
        log::info!(
            "Attaching files {:?} into file {}",
            files
                .iter()
                .map(move |f| f.path.display().to_string())
                .collect::<Vec<String>>(),
            result_file.display()
        );
        ...
        let playlist_filename = format!(
            "/tmp/tesla_playlist_tmp_{}_{}_{}.txt",
            now,
            when,
            camera.camera_file_name()
        );
        create_temp_playlist(files, playlist_filename.as_str())?;
        let result_tmp_file_path = self.folder.join(format!(
            "{}-tmp.mp4",
            result_file
                .file_stem()
                .and_then(|f| f.to_str())
                .ok_or(err_from_str(
                    format!("Cannot get file name for file {}", result_file.display()).as_str()
                ))?
        ));

        if result_tmp_file_path.exists() {
            log::info!(
                "Deleting temporary clip leftover: '{}'",
                &result_tmp_file_path.display()
            );
    ...
    pub fn create_mosaic(
        &self,
        file_cameras: &Vec<(String, Camera)>,
        mosaic_file: &PathBuf,
    ) -> io::Result<()> {
        log::info!("Composing mosaic clip '{}'", mosaic_file.display());
        let mosaic_file_name = mosaic_file
            .file_name()
            .ok_or(err_from_str(
                format!("Invalid file name '{}'", mosaic_file.display()).as_str(),
            ))
            .and_then(|f| f.to_str().ok_or(err_from_str("Invalid file name")))?;
        let temporary_mosaic_file = mosaic_file
            .parent()
            .ok_or(err_from_str(
                format!(
                    "Cannot find folder for mosaic file '{}'",
                    mosaic_file.display()
                )
                .as_str(),
            ))?
        ...
        let mut args = vec!["-y", "-filter_complex", filter_params.as_str()];
    ...
    pub fn mosaic_file(&self) -> io::Result<PathBuf> {
        ...
        Ok(self
            ...
            .parent()
            .ok_or(err_from_str(
                format!("Cannot find parent folder of {}", self.folder.display()).as_str(),
            ))?
    ...
    pub fn process(&self) -> () {
        ...
        match self.mosaic_file() {
            Err(e) => log::warn!(
                "Cannot calculate mosaic file name for clip folder {}: {}",
                self.folder.display(),
                e
            ),
            Ok(mosaic_file) if mosaic_file.exists() => log::info!(
                "Mosaic file '{}' already exists, skipping",
                mosaic_file.display()
            ),
            Ok(mosaic_file) => {
                let all_cameras = Camera::all_cameras().into_iter();
                let clip_files_and_camera: Vec<(String, Camera)> = all_cameras
                    .filter_map(|camera| {
                        self.concatenate_camera_files(&camera)
                            .ok()
                            .map(|f| (f, camera))
                    })
                    .collect();
                ...
                self.create_mosaic(&clip_files_and_camera, &mosaic_file)
                    .map_err(|e| {
                        log::error!(
                            "Error creating mosaic for clip {}: {}",
                            self.folder.display(),
                            e
                        )
                    })
...
fn process_folder(root: &DirEntry) -> io::Result<Vec<CameraFile>> {
    let mut clips: Vec<CameraFile> = list_files(root)?
        .into_iter()
        .filter_map(|e| match CameraFile::from(e.path().as_path()) {
            Ok(f) => Some(f),
            Err(err) => {
                log::error!(
                    "Found error processing clip {}: {}",
                    &e.path().display(),
                    err
                );
                None
    ...
    let children: Vec<DirEntry> = root
        ...
        .filter_map(|res| match res {
            Ok(e) => {
                ...
                if e.path().is_file() && e.path().extension().eq(&Some(OsStr::new("mp4"))) {
                    Some(e)
                } else {
                    None
                }
...
fn last_modified(entry: &DirEntry) -> io::Result<NaiveDateTime> {
    ...
    let mut modified_times: Vec<SystemTime> = files
        .into_iter()
        .filter_map(|f| f.metadata().and_then(|m| m.modified()).ok())
        .collect();
    modified_times.sort();
    let last_modified = modified_times
        .last()
        .and_then(|st| st.duration_since(SystemTime::UNIX_EPOCH).ok());
    last_modified
        .map(|l| l.as_secs() as i64)
        .map(|s| NaiveDateTime::from_timestamp(s, 0))
