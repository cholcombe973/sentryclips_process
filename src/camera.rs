...
impl Camera {
    ...
    pub fn from(name: &str) -> Option<Camera> {
        Camera::all_cameras()
            .into_iter()
            .find(|c| c.camera_file_name().eq(name))
    }
...
impl CameraFile {
    pub fn from(path: &Path) -> io::Result<CameraFile> {
        ...
        let capture = captures
            .next()
            .ok_or(err_from_str("Cannot identify the format of the file"))?;
        log::debug!(
            "Identified time of capture ({}) and camera ({})",
            &capture[1],
            &capture[2]
        );
        let date_time = parse_tesla_timestamp(&capture[1]).map_err(parse_error_to_io_error)?;
        let camera = Camera::from(&capture[2])
            .ok_or(err_from_str("Cannot identify camera from the file name"))?;
        Ok(CameraFile {
            path: path.to_owned(),
            camera,
